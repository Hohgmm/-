...
本專案包含一個瀏覽器版 GPT Crawler 設定工具。你可以在中文介面中編輯常用爬蟲設定、即時預覽產生的 `config.ts` 片段、複製設定，並依照流程產生可上傳到 OpenAI 的 `output.json`。

```sh
npm start
```

接著開啟 [http://localhost:3000](http://localhost:3000)。你也可以執行下列指令進行輕量驗證：
